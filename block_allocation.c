...
void set_block_allocation_table_name(const char *str) {
  if (file_name != NULL) {
    fprintf(
        stderr,
        "Cannot set %s as block allocation table name.\n"
...
static char *read_table() {
  if (file_name == NULL) {
    fprintf(stderr,
            "Failed to set the name of the block allocation table file.\n");
    exit(-1);
  }
...
static int write_table() {
  ...
  if (block_allocation_table == NULL) {
    fprintf(stderr, "Block allocation table has not been created in memory "
                    "before writing.\n");
    return -1;
  }
...
int allocate_block(int extent_size) {
  if (extent_size == 0) {
    ...
    fprintf(stderr, "Programming error: Trying to allocate extent that is 0 "
                    "blocks long.\n");
    return -1;
  }
  ...
  if (extent_size < 0) {
    // outside the permitted range
    fprintf(stderr, "Programming error: Trying to allocate extent of negative "
                    "size. Unrecoverable.\n");
    exit(-1);
  }
